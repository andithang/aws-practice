...
export const handler: APIGatewayProxyHandler = async (event) => {
  const requestFields = { lambda: 'admin-batch-publish', ...apiRequestLogFields(event) };
  ...
  try {
    ...
    await updateStatus(pk, sk, 'published');
    logInfo('Batch status updated to published', { ...requestFields, batchId, level, date });
